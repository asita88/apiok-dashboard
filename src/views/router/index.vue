...
export default {
  ...
  setup() {
    ...
    const getList = async params => {
      ...
      if (code != 0) {
        ...
        dataList.data.forEach(item => {
          // 方法列表
          let methods = ref([])
          if (item.request_methods && Array.isArray(item.request_methods) && item.request_methods.length > 0) {
          ...
          // 插件列表
          let plugins = ref([])
          if (item.plugin_list && Array.isArray(item.plugin_list) && item.plugin_list.length > 0) {
            item.plugin_list.forEach(pluginItem => {
              plugins.value.push({
                name: pluginItem.name,
                color: pluginItem.enable == 1 ? 'color-black' : 'color-light-grey',
                icon: pluginItem.icon || 'icon-apex_plugin1'
              })
            })
          }
          ...
          tmpList.value.push({
            ...
            upstream_res_id: item.upstream_res_id,
    ...
    const routerCopy = async record => {
      let { code, msg } = await $routerCopy(record.res_id)
