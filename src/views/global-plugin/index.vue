...
export default {
  ...
  setup() {
    ...
    const loadConfiguredPlugins = async () => {
      ...
      try {
        const result = await $globalPluginConfigList()
    ...
    const enableChange = async record => {
      ...
      try {
        const result = await $globalPluginConfigEnable(record.res_id, enableData)
        if (result.code !== 0) {
    ...
    const deleteConfig = async record => {
      try {
        const result = await $globalPluginConfigDelete(record.res_id)
        if (result.code !== 0) {
